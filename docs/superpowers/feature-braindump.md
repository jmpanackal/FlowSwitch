...
- add ability to change app behavior globally and per-profile and per window in the sidebar, changes should reflect in profile settings as well.
...
Canonical cross-doc snapshot lives in `docs/superpowers/unified-backlog.md`.
...
- **Status (2026-04-30):** Shipped hardening on this track: launch panel persistence during active runs, pin-at-launch preference, cancel IPC failure surfacing, truthful completion (placement verify/stabilize unless constrained acceptance), exe-directory `cwd` for spawns, fatal error dialogs treated as failures, constrained-placement warning path with clearer timeline copy. **Also shipped (post-run summary / inspector):** “Launch summary” header layout (outcome chip with title, profile + elapsed row, app counter), inspector shows the launched profile and polls with the active launch profile id during runs, per-app quiet OK vs warn/error pills with substeps/notes for non-OK, optional reuse line (“Reused existing window”), in-run overview counts, readable action states, completed-list outcome icons. **Still optional:** audio / taskbar progress, richer per-app hover in the summary list if needed after dogfood, and ongoing copy tuning.
- **Status (2026-05-04, `fix/audacity-confirmation-resume`):** confirmation-modal resume hardening landed for post-dismiss placement failures: FlowSwitch now logs post-modal candidate scans, rejects portrait/narrow companion panes in the ready gate, and escalates to launched-PID main-window discovery when lightweight enumeration only sees tool/pane windows (validated against the Audacity confirmation flow).
...
- **Progress (2026-05-01, `fix/content-launch-followups`):** app inspector **Associated Content** menu anchored for narrow sidebars (grid header row + `min(11rem,100%)` cap); host-aware association limits (`associatedContentHostLimits`, e.g. one folder root / unlimited files for VS Code–style hosts); removed obsolete inspect-root **`pt-12`** gap above app/content headers (Launch inspector unchanged).
- **Progress (2026-05-03, `fix/content-launch-followups`):** Explorer multi-folder tab automation hardening (Navigate2 / timing / no retry loop); placement + gather support; launch status clones `contentItems` + `contentSubstepMode`; renderer maps `contentItems` in `launchProgressFromStatus`; post-run summary lists each content path with `FileIcon`; timeline **`sub-content`** + row step **`opening-content`** for associated-content launches.
- **Progress (2026-05-05, `feature/explorer-windows-profile`):** Profile launch tracks HWNDs placed earlier in the same run so duplicate `explorer.exe` tiles cannot latch onto and move an Explorer window already assigned to another monitor (`placedHandlesInRun` wired through ready-gate exclusions, ranked/new-handle fallbacks, post-modal resume).
- **Release (2026-05-03):** Marketing site and root `package.json` bumped to **0.1.3**; push annotated **`v0.1.3`** after merge so CI attaches matching `FlowSwitch-0.1.3-win-x64-*.exe` to GitHub Releases.
- Scope:
